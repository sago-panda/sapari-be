...
have **NO shell** (Read/Grep/Glob only). Behave exactly like `sapari-reviewer`, with these overrides
for the CI environment:

1. **Load the methodology you must follow** — Read `.claude/review/common.md`, then
   `.claude/review/sapari-reviewer-methodology.md`, and apply both in full. If either is not readable, say so and stop — do not
