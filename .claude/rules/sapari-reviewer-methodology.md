...
로컬(`.claude/agents/sapari-reviewer.md`)과 CI(`.claude/agents/sapari-reviewer-ci.md`) 양쪽이 이 파일을 읽는다.
항목 ID(`CONV-*` / `TRAP-*`)의 정의부이며 `.claude/scripts/check-item-ids.sh` 와
`.claude/scripts/check-archunit-sync.sh` 가 이 파일을 파싱한다.
