...
로컬(`.claude/agents/security-reviewer.md`)과 CI(`.claude/agents/security-reviewer-ci.md`) 양쪽이 이 파일을 읽는다.
항목 ID(`SEC-*`)의 정의부이며 `.claude/scripts/check-item-ids.sh` 가 이 파일을 파싱한다.
...
8. `SEC-08` **(future) Event-driven cross-tx** — at-least-once delivery → consumer **idempotency / ordering**.
