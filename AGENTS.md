...
- `*-api` may depend on the shared foundation **only via `com.sapari.common.page`** (CursorPage/OffsetPage in use-case return types); `common/global` (TimeProvider, …) must NOT leak into `-api`.
