...
public class CustomerJwtSessionService {
     ...
     */
    public IssuedTokenPair issueTokenPair(UserView customer) {
        UUID sessionId = UUID.randomUUID();
        JwtSubject subject = toJwtSubject(customer, sessionId);
     ...
     */
    public String replaceAccessTokenForNickname(AccessSession accessSession, UserView savedCustomer) {
        blacklistAccessToken(accessSession);

        return jwtTokenProvider.createAccessToken(toJwtSubject(savedCustomer, accessSession.sessionId()));
    }
    ...
    /**
     * JWT 파싱 실패를 구매자 인증 도메인 예외로 변환한다.
     */
     ...
     */
    private void validateRefreshSessionOwner(RefreshSession refreshSession, UserView customer) {
        if (!refreshSession.userId().equals(customer.userId())) {
            throw new CustomerException(CustomerErrorCode.INVALID_REFRESH_TOKEN);
        }
    }
