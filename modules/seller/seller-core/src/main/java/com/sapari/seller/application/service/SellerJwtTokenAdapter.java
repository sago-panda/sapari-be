...
/**
 * 판매자 인증 flow가 공통 JWT 토큰 생명주기를 사용할 수 있게 연결하는 도메인 래퍼.
 * common 모듈은 SellerException/UserView를 모르므로 여기서 변환한다.
 */
@Component
@RequiredArgsConstructor
public class SellerJwtTokenAdapter {
    ...
    /**
     * Refresh Token 검증 실패를 판매자 도메인 예외로 변환한다.
     */
    ...
    /**
     * 공통 RTR 처리 결과를 그대로 사용하되 실패 응답 코드는 판매자 도메인이 결정한다.
     */
    public JwtTokenLifecycle.RotatedRefreshToken rotateRefreshToken(
        ...
        try {
            return jwtTokenLifecycle.rotateRefreshToken(refreshSession, toJwtTokenPrincipal(seller));
        } catch (JwtTokenLifecycleException e) {
            throw invalidRefreshToken(e);
        }
    }
    ...
    /**
     * Access Token 검증 실패를 판매자 도메인 예외로 변환한다.
     */
    ...
    public String replaceAccessTokenForNickname(
            ...
            UserView savedSeller
    ) {
        try {
            return jwtTokenLifecycle.replaceAccessToken(accessSession, toJwtTokenPrincipal(savedSeller));
        } catch (JwtTokenLifecycleException e) {
            throw invalidAccessToken(e);
        }
    }

    private JwtTokenPrincipal toJwtTokenPrincipal(UserView seller) {
    ...
    private SellerException invalidRefreshToken(Throwable cause) {
        return new SellerException(SellerErrorCode.INVALID_REFRESH_TOKEN, cause);
    }

    private SellerException invalidAccessToken(Throwable cause) {
        return new SellerException(SellerErrorCode.INVALID_ACCESS_TOKEN, cause);
    }
}
