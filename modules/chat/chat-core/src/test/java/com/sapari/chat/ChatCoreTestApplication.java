 ...
 *
 * <p>application.service(@Service)는 스캔에서 제외한다 — SendChatService 등은 {@code ChatBroadcaster}처럼
 * 소비 앱(streaming-app=T9)이 제공하는 어댑터에 의존해 chat-core 단독 컨텍스트로는 와이어가 불가능하다(헥사고날).
 * 서비스는 Mockito 단위테스트로 검증하므로 통합 부트 컨텍스트(@SpringBootTest=Mongo 영속 테스트)에 넣을 필요가 없다.
 *
...
@ComponentScan(excludeFilters = @ComponentScan.Filter(
        ...
        pattern = "com\\.sapari\\.chat\\.application\\.service\\..*"))
