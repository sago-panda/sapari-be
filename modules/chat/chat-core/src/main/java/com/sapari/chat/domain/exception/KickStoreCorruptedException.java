 ...
 * <b>읽기와 쓰기가 이 예외를 다르게 쓴다.</b> 조회 경로는 fail-open으로 흡수해 클라이언트까지 가지
 * 않고 로그 등급만 올린다. 등록 경로는 흡수할 것이 없어 그대로 던진다 — 등록하지 못한 강퇴는
 * 일어나지 않은 강퇴다. (그 예외가 HTTP 응답이 되는 것은 강퇴 유스케이스가 배선된 뒤다.)
 *
 * <p><b>치워야 할 키 이름은 로그에만 실린다</b> — 응답 문구는 에러코드가 갖고 있어 내부 식별자가
 * 밖으로 새지 않는다.
 *
